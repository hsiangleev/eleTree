 ...
 */
export default function changeParent<D extends ITreeData>(thisTree: EleTree<D>, cData: D, isRecall = false) {
  ...
  let f = (v: D) => { return !isUndefined(v[symbolAttr.disabledParentStatus]) ? v[symbolAttr.disabledParentStatus] : v[checked] }
  ...
  let filterData = pData[children].filter((v: D) => fn(v))
  let s = filterData.length === 0
    ? pData[checked]
    : filterData.every((v: D) => f(v) === 2)
      ? 2
      : filterData.some((v: D) => f(v) === 2 || f(v) === 1)
        ? 1 : 0
  ...
  isRecall && changeParent(thisTree, pData, isRecall)
}
