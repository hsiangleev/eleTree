...
export function renderData(options, isFirstRender = false) {
    let indexArr = []
    let pData = {}
    changeData(options, options.data, pData, indexArr, isFirstRender)
}
/**
 * 
 * @param {*} options 
 * @param {*} data 需要修改的原始数据数组
 * @param {*} pData 父节点数据
 * @param {*} indexArr 父节点索引
 * @param {*} isFirstRender 是否是初始渲染（初始渲染选中状态和下拉状态由多种条件组成，之后的状态只由当前节点状态决定）
 */
export function changeData(options, data, pData, indexArr, isFirstRender = false) {
    ...
    data.forEach((v, i)=>{
        ...
        if(!options.checkStrictly){
            ...
            pStatus = pData[disabled] ? pData[symbolAttr.disabledParentStatus] === 2 : pData[checked] === 2
        ...
        if(v[options.request['children']]) {
            changeData(options, v[children], v, indexArr, isFirstRender)
        }
