...
ele.on('lazyload', function (d: any) {
  let { data, load } = d
...
ele.on('checkbox', function (data: any) {
  // console.log(this)
  // console.log(data)
})
ele.on('click', function (this: Event, data: any) {
  const target = this.target as HTMLElement
  if (target.classList.contains('addchild_test')) {
    ele.append(data.data.id, {
      label: 'aaa',
      id: ++index
    })
    ele.edit(index, 'add_child')
  } else if (target.classList.contains('edit_test')) {
    ele.edit(data.data.id)
  } else if (target.classList.contains('delete_test')) {
